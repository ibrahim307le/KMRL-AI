...
CHUNK_SIZE = 2000  # chars per chunk for training
CHUNK_OVERLAP = 200
...
def process_and_bootstrap(pdf_path, export_for_labeling=True, chunk_size=CHUNK_SIZE):
    ...
    if export_for_labeling:
        # create a user-friendly file with text trimmed
...
def classify_and_route(pdf_path, model_dir, summarizer_model=SUMMARIZER_MODEL):
    """
    Use the saved model to classify each page/chunk of pdf_path,
    group pages by department, create per-department PDF (derived),
    save text and a summary pdf for each department.
    """
    ensure_output_dirs()
    pdf_path = Path(pdf_path)
    dest_original = ORIGINALS_DIR / pdf_path.name
